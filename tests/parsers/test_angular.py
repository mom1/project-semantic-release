...
def test_parser_return_correct_bump_level():
    assert angular_parser("feat(parsers): Add new parser pattern\n\nBREAKING CHANGE: ")[0] == 3
    assert angular_parser("feat(parsers)!: Add new parser pattern\n\nBREAKING CHANGE: ")[0] == 3
    assert (
        angular_parser("feat(parsers): Add new parser pattern\n\n" "New pattern is awesome\n\nBREAKING CHANGE: ")[0]
        == 3
    )
    assert angular_parser("feat(parsers): Add new parser pattern\n\nBREAKING-CHANGE: change !")[0] == 3
...
def test_parser_return_subject_from_commit_message():
    ...
    assert angular_parser("fix(parser): Fix regex in angular parser")[3][0] == "Fix regex in angular parser"
    assert angular_parser("test(parser): Add a test for angular parser")[3][0] == "Add a test for angular parser"
