...
def test_version_by_commit_should_call_correct_functions(mocker):
    ...
    mock_evaluate_bump = mocker.patch("semantic_release.cli.evaluate_version_bump", return_value="major")
    mock_current_version = mocker.patch("semantic_release.cli.get_current_version", return_value="1.2.3")
...
def test_print_version_no_change(mocker, runner, capsys):
    ...
    mock_evaluate_bump = mocker.patch("semantic_release.cli.evaluate_version_bump", return_value=None)
    mock_current_version = mocker.patch("semantic_release.cli.get_current_version", return_value="1.2.3")
...
def test_print_version_change(mocker, runner, capsys):
    mock_current_version = mocker.patch("semantic_release.cli.get_current_version", return_value="1.2.3")
    mock_evaluate_bump = mocker.patch("semantic_release.cli.evaluate_version_bump", return_value="minor")
...
def test_version_check_build_status_fails(mocker):
    mock_check_build_status = mocker.patch("semantic_release.cli.check_build_status", return_value=False)
    mock_tag_new_version = mocker.patch("semantic_release.cli.tag_new_version")
...
def test_version_retry_and_giterror(mocker):
    mocker.patch("semantic_release.cli.get_current_version", mock.Mock(side_effect=GitError()))
...
def test_publish_retry_version_fail(mocker):
    mock_get_current = mocker.patch("semantic_release.cli.get_current_version", return_value="current")
    mock_get_previous = mocker.patch("semantic_release.cli.get_previous_version", return_value="previous")
...
def test_publish_giterror_when_posting(mocker):
    mock_get_current = mocker.patch("semantic_release.cli.get_current_version", return_value="current")
    mock_evaluate = mocker.patch("semantic_release.cli.evaluate_version_bump", return_value="patch")
    mock_get_new = mocker.patch("semantic_release.cli.get_new_version", return_value="new")
    mock_get_owner_name = mocker.patch(
        "semantic_release.cli.get_repository_owner_and_name",
        return_value=("owner", "name"),
...
def test_overload_by_cli(mocker, runner):
    mock_read_text = mocker.patch("semantic_release.history.Path.read_text", mock_version_file)
    runner.invoke(
        main,
        [
            "version",
