...
class RepositoryTests(TestCase):
    ...
    def test_raises_error_when_repo_name_invalid(self, mock_handle_creds):
        repo = ArtifactRepo(Path("dist"))
        with self.assertRaises(ImproperConfigurationError, msg="Upload to artifact repository has failed"):
    ...
    def test_upload_with_custom_settings(self, mock_upload):
        repo = ArtifactRepo(Path("custom-dist"))
        repo.upload(noop=False, verbose=True, skip_existing=True, comment="distribution comment")
        ...
        self.assertEqual(settings.repository_config["repository"], "https://custom-repo")
