...
class Gitlab(Base):
    ...
    def post_release_changelog(cls, owner: str, repo: str, version: str, changelog: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Release {ref} could not be created for project {owner}/{repo}")
            return False
    ...
    def post_pull_request(
        ...
        **kwargs,
        ...
        gl = GitlabObj(Gitlab.api_url(), private_token=Gitlab.token(), ssl_verify=not kwargs.get("insecure"))
        project_str = f"{owner}/{repo}"
        try:
            project = gl.projects.get(project_str)
        except exceptions.GitlabAuthenticationError:
            logger.warning(
                f"Unable to get project {project_str}." "Unauthorised access, check your access token is valid."
            )
            return False
        ...
        existing_mr = next(
            (True for mr in mrs if mr.source_branch == source_branch and mr.target_branch == target_branch),
            None,
        )
        if existing_mr:
            logger.debug(
...
def post_pull_request(
    ...
    **kwargs,
    ...
    return get_hvcs().post_pull_request(owner, repository, title, changelog, source_branch, target_branch, **kwargs)
